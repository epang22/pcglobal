function PCfit_snobfit_pseudo( data )
%PCFIT_SNOBFIT_PSEUDO Fit PC for a single pattern by snobfit in 6d space,
% also considers pseudosymmetry angles
...
% data.outputprefix = 'output_1';   % Names of output files are [outputprefix]_v#.h5/.txt (located in path)
% 
...
% full path to pattern image file
...
if exist(img_exp_path,'file')==2
    ...
    % define paths
    h5path = fullfile(homepath,path,[outputprefix '.h5']);    % full path to h5 output file
    logpath = fullfile(homepath,path,[outputprefix '.txt']);    % full path to txt log output file
    ...
    if exist(h5path,'file')==2 || exist(logpath,'file')==2
        while exist(h5path,'file')==2 || exist(logpath,'file')==2
            h5path = [h5path(1:end-3) '_1.h5'];
            logpath = [logpath(1:end-4) '_1.txt'];
        end
...
%     end
    ...
    %%% loop through pseudosymmetry variants if pseudosym variants are defined
    if exist('pseudosym','var')==1
        for ii=1:size(pseudosym,1)
            totaltime = tic;
            
            % print which variant
            fprintf('\n  === Pseudosymmetry variant %g of %g ===\n\n',ii,size(pseudosym,1));

            % define paths
            h5path = fullfile(homepath,path,[outputprefix '_v' num2str(ii) '.h5']);    % full path to h5 output file
            logpath = fullfile(homepath,path,[outputprefix '_v' num2str(ii) '.txt']);    % full path to txt log output file
            ...
            % figure out starting euler angles for this pseudosym variant
            Reuler = eu2om([phi1 PHI phi2]*pi/180);   % rotation matrix
            Rrot1 = axang2rotm([pseudosym(ii,1:3) pseudosym(ii,4)*pi/180]);  % Matrix for specified rotation
            R1 = Rrot1*Reuler;                	% Rotation matrix for Euler angles + specified rotation
            eulerend1 = om2eu(R1);       % Euler angles (rad) for pseudosymmetry variant
            q1 = eu2qu(eulerend1);     % Quaternion of starting Euler angles (center of grid)
